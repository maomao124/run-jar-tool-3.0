...
void run_args_save(string pathfile, bool ishide, char* argv[], string path)
{
	vector<string> java_args;
	string strcmd = "java -jar ";
	cout  << "  即将执行的jar文件的名称：";
	cout << pathfile << endl;
	string pathfile2 = pathfile;
	string ja = "_args_save";
	solve(pathfile, ja);
	string ja1 = ".jar";
	solve(pathfile, ja1);
	string ja2 = "_hide";
	solve(pathfile, ja2);
	path = path +"args_" + pathfile + ".ini";
	//cout << path << endl;
	ifstream argsini(path, ios::in);
	if (!argsini)
	{
		cout << "  请输入参数的个数：";
		int num = 0;
	loop:
		...
		for (int i = 0; i < num; i++)
		{
			cout << "  请输入第" << i + 1 << "个参数：";
			cin >> args;
			java_args.push_back(args);
		}
		strcmd = strcmd + pathfile2;
		for (int i = 0; i < num; i++)
		{
			//cout << java_args[i] << endl;
			strcmd = strcmd + " " + java_args[i];
		}
		ofstream argsiniwrite(path, ios::out);
		argsiniwrite << java_args[0];
		for (int i = 1; i < num; i++)
		{
			argsiniwrite << " " << java_args[i];
		}
		argsiniwrite.close();
	}
	else
	{
		...
		{
			argsini >> sss;
			java_args.push_back(sss);
		}
		cout << "  传入的数组：";
		cout << java_args[0];
		for (int i = 1;i < java_args.size();i++)
		{
			cout <<" " << java_args[i];
		}
		cout << endl;
	...
	if (pathfile2.find("_hide") < pathfile2.length() - 3)
	{
		ishide = true;
		HWND hwnd;
...
void run_args(string pathfile, bool ishide, char* argv[])
{
	vector<string> java_args;
	cout << "  即将执行的jar文件的名称：";
	cout << pathfile << endl;
...
int main(int args, char* argv[])
{
	...
	*/
	...
	cout << setw(40) << "jar文件启动器3.0   by mao" << endl;
	SetConsoleColor(FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_INTENSITY);
	//vector<string> files;
	...
	if (args == 2)
	{
		...
		if (pathfile.find("_args_save") < pathfile.length() - 3)
		{
			run_args_save(pathfile, ishide, argv,path);
		}
		else if (pathfile.find("_args") < pathfile.length() - 3)
		{
			run_args(pathfile,ishide,argv);
		}
	...
	{
		path = path + "\\";
		cout << "  传入了多个jar文件的路径，请选择要运行的文件" << endl;
		cout << "-------------------------------" << endl;
		string s2;
	...
	loop:
		...
		if (s3.find("_args_save") < s3.length() - 3)
		{
			run_args_save(s3, ishide, argv, path);
		}
		else if (s3.find("_args") < s3.length() - 3)
		{
			run_args(s3, ishide, argv);
		}
	...
	else if (args == 1)
	{
		...
		cout << "        当jar的文件名称包含 \"_args\" 关键字时，可传入参数" << endl;
		cout << "        当jar的文件名称包含 \"_args_save\" 关键字时，传入参数并保存，下次启动时无需再次输入" << endl;
		SetConsoleColor(FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_INTENSITY);
			...
			else
			{
				ifstream log("C:\\tools\\jar文件启动器\\run jar.log", ios::in);
				{
					if (!log)
					{
						SetConsoleColor(FOREGROUND_RED | FOREGROUND_INTENSITY);
						cout << "没有找到日志文件！！！" << endl;
						cout << "默认路径为 C:\\tools\\jar文件启动器\\run jar.log" << endl;
						SetConsoleColor(FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_INTENSITY);
					}
					else
					{
